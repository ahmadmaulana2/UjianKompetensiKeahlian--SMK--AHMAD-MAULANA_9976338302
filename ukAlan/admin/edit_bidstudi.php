...
if ((isset($_POST["MM_update"])) && ($_POST["MM_update"] == "form1")) {
  $updateSQL = sprintf("UPDATE bidang_studi SET bidang_nama=%s WHERE bidang_kode=%s",
                       ...
                       GetSQLValueString($_POST['bidang_kode'], "char"));
...
if (isset($_GET['bidang_kode'])) {
  $colname_bidstudi = $_GET['bidang_kode'];
}
mysql_select_db($database_koneksi, $koneksi);
$query_bidstudi = sprintf("SELECT * FROM bidang_studi WHERE bidang_kode = %s", GetSQLValueString($colname_bidstudi, "char"));
$bidstudi = mysql_query($query_bidstudi, $koneksi) or die(mysql_error());
$row_bidstudi = mysql_fetch_assoc($bidstudi);
$totalRows_bidstudi = mysql_num_rows($bidstudi);
...
<li><a class="MenuBarHorizontal" href="#">Input</a>
	...
	<div id="site_content">		
	  ...
	  <div id="content">
        ...
        <h1><div align="center">EDIT DATA BIDANG STUDI</div></h1>
        <div align="center"><form action="<?php echo $editFormAction; ?>" method="post" id="form1" enctype="multipart/form-data" name="form1">
          <table border="23" align="center" cellpadding="5" cellspacing="5">
            <tr valign="baseline">
              <td align="right"><strong>Kode Bidang :</strong></td>
              <td><?php echo $row_bidstudi['bidang_kode']; ?></td>
            </tr>
            <tr valign="baseline">
              <td align="right"><strong>Nama Bidang :</strong></td>
              <td><input type="text" name="bidang_nama" value="<?php echo htmlentities($row_bidstudi['bidang_nama'], ENT_COMPAT, 'utf-8'); ?>" size="32" /></td>
            </tr>
            <tr valign="baseline">
              <td align="right">&nbsp;</td>
          ...
          <input type="hidden" name="bidang_kode" value="<?php echo $row_bidstudi['bidang_kode']; ?>" />
        </div>
        </form>
